...
//$result = $myip->find('39.171.128.37');
//var_dump($result);
...
class Ipipnet
{
    ...
    public static function find($ip,$redis='')
    {

        //先读取redis里面的
	    if(!empty($redis)){
            $redis->databaseSelect('Ip');
	        $res = $redis->hget('Ip:'.$ip);
	        if($res) {
	            $result_arr = [1=>$res['province'],2=>$res['city']];
	            return $result_arr;
	        }
	    }
        if (empty($ip) === TRUE)
        ...
        {
            if(!empty($redis)){
                $redis_district = ['province'=>self::$cached[$nip][1],'city'=>self::$cached[$nip][2]];
                $redis->hset('Ip:'.$ip, $redis_district);
                $redis->expire('Ip:'.$ip, 600);
            }
