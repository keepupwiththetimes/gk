<?php
namespace Common\Api\Wxcomponent;
/**
...
class Wechat
{
	...
	public function getRev()
	{
		...
		file_put_contents("debug.log", "////////getRev".$postStr."/////////\n",FILE_APPEND);
		//兼顾使用明文又不想调用valid()方法的情况
		$this->log($postStr);
		if (!empty($postStr)) {
			$this->_receive = (array)simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
			file_put_contents("debug.log", "////////getRev222".json_encode($this->_receive) ."/////////\n",FILE_APPEND);
		}
		return $this;
	}
