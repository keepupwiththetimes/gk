...
class PlatformProductModel extends Model
{
    ...
    public function getProductList($type = 1, $start = 0, $num = 10000, $key = "", $category_id = "", $order_by = "", $select_type = 1)
    {
        ...
        switch ($order_by) {
            case 'priceinc':
                $order = $tablePrefix . "product.coupon_value ASC";
                break;
            case 'pricedesc':
                $order = $tablePrefix . "product.coupon_value DESC";
                break;
            case 'saleinc':
            ...
            case 'saledesc':
                ...
                break;
            case 'dateinc':
                $order = $tablePrefix . "product.start ASC";
                break;
            case 'datedesc':
                $order = $tablePrefix . "product.start DESC";
                break;
            // case 'popularityinc':
        ...
        $product_list = $this->join($tablePrefix . 'product on ' . $tablePrefix . 'platform_product.product_id=' . $tablePrefix . 'product.id')
            ->field($tablePrefix . 'product.*,' . $tablePrefix . 'platform_product.type,' . $tablePrefix . 'platform_product.salequantity, FORMAT(' .  $tablePrefix . 'product.coupon_value, 0 ) as price_value')
            ->where($where)
            ->order($order)
            ->limit($start, $num)
            ->cache(true, 60)
...
//             检查商品是否已经售完(检测当卡平台奖品是否发完)
    ...
    /**
     * [getProductAllSaleQuantity 获取所有的销售数量]
     * @return [type] [description]
     */
    public function getProductAllSaleQuantity(){
        $where = "status = 1";
        $product_info = $this->table($this->tablePrefix . "product")
                    ->field($this->tablePrefix . 'product.id, ' . $this->tablePrefix . 'product.saleallquantity')
                    ->where($where)
                    ->select(); 
        return $product_info;          
    }
    ...
    /**
     * [getproductPointByUserProduct 获取该平台所有产品id]
     * @return [type] [description]
     */
    public function getproductcollectByUserProduct($platform_id){
        //$platform_id = intval($_SESSION[$_SESSION['PLATFORM_CODE']]['platform_id']);
        $where = $this->tablePrefix . 'platform_product.type = 1 and ' . $this->tablePrefix . 'product.status=1 and ' . $this->tablePrefix . 'platform_product.status=1 and ' . $this->tablePrefix . 'platform_product.platform_id =' . $platform_id;
        $product_list = $this->join($this->tablePrefix . 'product on ' . $this->tablePrefix . 'platform_product.product_id=' . $this->tablePrefix . 'product.id')
            ->field($this->tablePrefix . 'product.id')
            ->where($where)
            ->select(); 
        return $product_list;
    }
